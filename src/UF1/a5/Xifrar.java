...
public class Xifrar {
    ...
    public static byte[][] encryptWrappedData(byte[] data, PublicKey pub) {
        ...
        try {
            //algoritmo de la generacion de clave
            KeyGenerator kgen = KeyGenerator.getInstance("AES");
            // clave tamaño
            kgen.init(128);
            // clau simetrica
            ...
            // "activar modo encrypt" con la clave simetrica
            cipher.init(Cipher.ENCRYPT_MODE, sKey);
            // dato xifrado
            byte[] encMsg = cipher.doFinal(data);
            // algoritmo de clave publica
            cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding",  "SunJCE");
            // "activar modo wrap" con la clave publica
            cipher.init(Cipher.WRAP_MODE, pub);
            // clau simetrica enrollado
    ...
    public static byte[] decryptWrappedData(byte[] encryptedMessage, PrivateKey privateKey,byte[] encryptedKey) {
        try {
            ...
            Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding",  "SunJCE");
            // "activar modo unwrap" con la clave privada
            cipher.init(Cipher.UNWRAP_MODE, privateKey);
            // clave xifrada desenrollado
            Key symmetricKey = cipher.unwrap(encryptedKey, "AES", Cipher.SECRET_KEY);
            // algoritmo de clave simetrica
            cipher = Cipher.getInstance("AES");
            // "activar modo decrypt" con la clave simetrica
            cipher.init(Cipher.DECRYPT_MODE, symmetricKey);
            // devolver dato dexifrado
